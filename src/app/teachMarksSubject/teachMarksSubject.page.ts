//jab mein wapis this.loadfunction(); call karta hu to meri UpdateMarksSetting call kyu ho rahi hai and kaha call ho rahi hai 
...
@Component({
  selector: 'app-teachMarksSubject',
  templateUrl: './teachMarksSubject.page.html',
  styleUrls: ['./teachMarksSubject.page.scss'],
})
export class TeachMarksSubjectPage implements OnInit {
...
this.cancel('modal-' + SubjectID);
    this.modal.dismiss({
      ...
      markty: this.markty,
      include_in_gt: this.include_in_gt,
...
ChangeValue(event, subidss, levelid) {
  ...
  const newIncludeInGTTotal = event.detail.checked ? 'Y' : 'N';

  
  // if (this.IncludeInGTTotal === newIncludeInGTTotal) {
  //   return;
  // }

  
   this.IncludeInGTTotal = newIncludeInGTTotal;
